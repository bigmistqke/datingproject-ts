...
function BlockManager(_ext) {
    ...
    _position.addEventListener('update', (e) => { console.log('update'); updateBlock(e.detail.block_id, { position: e.detail.position }) })
    ...
    const openRoleOverlay = async (e, block) => {
        ...
        let result = await openOverlay({ type: 'role', data: { block: block, roles: roles, position: { x: e.clientX, y: e.clientY } } });
        _ext.setOverlay(false);
        if (!result) return;
