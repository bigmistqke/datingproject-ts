...
function Role(props: {
  ...
  updateRoleOffset: any
  in_outs: InOuts
  all_roles: Record<string, RoleType>
  role_hue: number | undefined
  description: string
  name: JSXElement
}) {
  const [state, actions] = useStore()

  const [getDelay, setDelay] = createSignal<number>()

  let is_initialized = false
  let role_dom: HTMLElement

  const startConnection = async (e: MouseEvent, role_id: string) => {
  ...
  const convertRole = async () => {
    ...
    let options = Object.entries(remaining_roles).map(
      ([role_id, role]) => {
        return {
          value: role_id,
    ...
    let [target_role_id] = await actions.openPrompt({
      ...
      header: (
        <>
          ...
          <div>
            from <span class="selected_nodes">selected node</span> into:
          </div>
        </>
      ),
      data: { options },
  ...
  const removeOrConvertRole = async (e: MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    ...
    let result = await actions.openPrompt({
      ...
      header: (
        <>
          ...
          <div>and its associated instructions</div>
          <div>
            from <span class="selected_nodes">selected node</span>
          </div>
        </>
      ),
      data: {
  ...
  return (
    <RolesContainer>
      <Bubble
        ...
        onPointerDown={e => startConnection(e, props.role_id)}
        onContextMenu={e => removeOrConvertRole(e)}
