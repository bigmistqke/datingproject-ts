...
function Role(props) {
  ...
  const startConnection = async (e, role_id) => {
    ...
    } else if (target.classList.contains('map-container') && state.editor.bools.isShiftPressed) {
  ...
  const updateRoleOffset = () => {
    ...
    actions.updateRoleOffset({
      ...
      offset: {
        ...
        y: props.direction === 'out' ? role_dom.offsetTop + 5 : role_dom.offsetTop, // size of the border around the node
  ...
  const removeOrConvertRole = async e => {
    ...
    let result = await actions.openPrompt({
      ...
      header: (
        <>
          <div>
            remove or convert role <Bubble background_hue={props.role_hue}>{props.role_id}</Bubble>
          </div>
          <div>and its associated instructions</div>
          <div>
