...
import { array_insert, array_remove_element } from '../utils/pure-array'

import getRandomHue from '../utils/getRandomHue'
import arrayOfObjectsToObject from '../utils/arrayOfObjectsToObject'
...
export default function ScriptActions({ state, setState, actions }) {
  ...
  this.setScriptId = script_id =>
    setState('script', 'script_id', script_id)
  this.setDesignId = design_id =>
  ...
  const getEndNode = async ({ node_id, role_id }) => {
    let { traversed_node_ids } = await this.traverseRole({
      node_id,
      role_id,
    })
  ...
  this.mergeSelectedNodes = () => {
    ...
    let all_affected_role_ids = all_in_outs.map((in_outs, i, role_ids) =>
      Object.keys(in_outs).filter(
        role_id => role_ids.indexOf(role_id) === -1,
      ),
    )
