...
export default function ScriptActions({ state, setState, actions }) {
  ...
  this.setInstructions = instructions => setState('script', 'instructions', instructions)
  ...
  const offsetConnections = ({ node_ids, offset }) => {
    let _connections = connections.map(_connection => {
      ...
      if (node_ids.includes(_connection.in_node_id)) {
        if (!_pos[0]) {
          console.error('_connection.in_node_id', _connection.in_node_id, connections)
        } else {
          _pos[0].x += offset.x
          _pos[0].y += offset.y
        }
      ...
      if (node_ids.includes(_connection.out_node_id)) {
        if (!_pos[1]) {
          console.error('_connection.out_node_id', _connection.out_node_id, connections)
        } else {
          _pos[1].x += offset.x
          _pos[1].y += offset.y
        }
  ...
  const removeNode = node_id => {
    ...
    batch(() => {
      ...
      setState('editor', 'selection', selection => selection.filter(id => id !== node_id))
  ...
  const setConnection = ({ node_id, connecting_node_id, role_id, direction }) => {
  ...
  this.duplicateSelectedNodes = () => {
    batch(() => {
      ...
      state.editor.selection.forEach(node_id => (node_map[node_id] = uniqid()))
      ...
      Object.values(node_map).forEach(new_node_id => {
        Object.entries(state.script.nodes[new_node_id].in_outs).forEach(([role_id, in_out]) => {
          if (in_out.out_node_id) {
            if (state.editor.selection.indexOf(in_out.out_node_id) === -1) {
              setState('script', 'nodes', new_node_id, 'in_outs', role_id, 'out_node_id', undefined)
            } else {
              setState(
                'script',
                'nodes',
          ...
          if (in_out.in_node_id) {
            if (state.editor.selection.indexOf(in_out.in_node_id) === -1) {
              setState('script', 'nodes', new_node_id, 'in_outs', role_id, 'in_node_id', undefined)
            } else {
              setState(
                'script',
                'nodes',
  ...
  this.removeRoleFromNode = ({ node_id, role_id }) => {
    let instruction_ids = state.script.nodes[node_id].instructions.filter(
      instruction_id => state.script.instructions[instruction_id].role_id === role_id,
    )
  ...
  this.convertRole = ({ node_ids, source_role_id, target_role_id }) => {
    ...
    node_ids.forEach(node_id => {
      ...
      node.instructions
        .filter(
          instruction_id => state.script.instructions[instruction_id].role_id === source_role_id,
        )
      ...
      if (source_in_out.in_node_id) {
        // check if
        if (state.script.nodes[source_in_out.in_node_id].in_outs[target_role_id]) {
          reconnections.push({
            node_id,
  ...
  this.addInstructionIdToNode = ({ node_id, instruction_id, index = false }) => {
    ...
    if (index) {
      ...
      setState('script', 'nodes', node_id, 'instructions', [...instruction_ids, instruction_id])
    }
  }
  ...
  this.addConnection = ({ node_id, connecting_node_id, role_id, direction }) => {
  ...
  this.removeConnectionBothWays = ({ node_id, role_id, direction }) => {
    let connecting_node_id = state.script.nodes[node_id].in_outs[role_id][prevOrNext(direction)]
  ...
  this.setDescriptionScript = description => setState('script', 'description', description)
  ...
  const getPrevInstructionIds = ({ node, index }) => [node.instructions[index - 1]]
    ...
    try {
      ...
      text.split('').forEach(char => {
        ...
        if (parsed_text.length === 0) {
          ...
          let type = 'normal'
          if (char === '[') {
            if (!previous_node.open) throw 'no nesting allowed'
            ...
            if (!previous_node.open) throw 'the textnode before ) has already been closed by a )'
            ...
            if (previous_node.open) previous_node.text += char
            else parsed_text.push({ type: 'normal', text: char })
          }
        }
      })
          ...
          .map(([node_id, node]) =>
            node.instructions
              ...
              .map((instruction_id, index) => {
                ...
                if (instruction.type !== 'video') instruction.text = formatText(instruction.text)
  ...
  this.processScript = async () => {
    try {
      ...
      if (errored_roles.length > 0) throw [`controlRoles had errors:`, errored_roles]
        ...
        Object.entries(state.script['roles']).map(([role_id, role]) => [
          role_id,
          {
            ...
            instructions: processed_roles[role_id].node_ids
              .map(node_id =>
                state.script.nodes[node_id].instructions
                  .filter(instruction_id => instructions[instruction_id].role_id === role_id)
                  .map(instruction_id => {
                    const instruction = {
  ...
  this.mergeSelectedNodes = () => {
    const selected_nodes = state.editor.selection.map(node_id => [
      node_id,
      state.script.nodes[node_id],
    ])
  ...
  this.createGame = async () => {
    try {
      ...
      const { error } = await postData(`${urls.fetch}/api/script/test/${state.script.script_id}`, {
        roles,
        design_id: state.script.design_id,
      })
  ...
  this.fetchScript = async () => {
    let data = await getData(`${urls.fetch}/api/script/get/${state.script.script_id}/development`)
      ...
      Object.entries(instructions)
        .filter(
          ([instruction_id, instruction]) =>
            instruction.type === 'video' && instruction.text !== '' && !instruction.filesize,
        )
