...
const Element = ({ id, element, viewport, guides, card_dim, shouldSnap, shiftPressed, altPressed, typeInFocus, loremIpsum }) => {
    ...
    const Type = ({ element }) => {
        ...
        switch (element.type) {
            case 'image':
                return <img src={isDataURL(element.src) ? element.src : `${window._url.fetch}/${element.src}`}></img>;
            case 'text_type':
