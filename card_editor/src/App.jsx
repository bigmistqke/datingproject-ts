...
function App(props) {
  ...
  createEffect(() => {
    ...
    setInstruction(
      'text',
      state.viewport.modes.choice ? lorem_ipsum.choice[0] : lorem_ipsum.normal[0],
    )
  })
  ...
  const fetchDesign = async design_id => {
    try {
      let result = await fetch(`${urls.fetch}/api/design/get/${design_id}/development`)
    ...
    <>
      ...
      >
        ...
        <Viewport
          ...
          onDragOver={e => e.preventDefault()}
          onDragEnter={e => e.preventDefault()}
          onMouseDown={e => (e.buttons === 1 ? actions.setSelectedElementIndex(false) : null)}
        >
          <Rulers
