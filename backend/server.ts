...
// fetch time on server
app.get('/api/getServerTime', (req, res) => res.json({ timestamp: new Date().getTime() }))

// Script-Endpoints: /api/script/*
const initScriptEndpoints = () => {
  // save script
...
// Design-Endpoints: /api/design/*
const initDesignEndpoints = () => {
  ...
  // error while fetching all designs
...
const initVideoEndpoints = () => {
  // upload video
  app.post('/api/video/upload/:script_id/:type', async function (req, res, next) {
    console.info(req.path, JSON.stringify(req.params))
  ...
  app.get('/api/video/download/:file_name', async (req, res, next) => {
    console.info(req.path, JSON.stringify(req.params))
