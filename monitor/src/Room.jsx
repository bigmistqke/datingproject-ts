...
export default function Room(props) {
  ...
  let sorted_players = createMemo(() => {
    ...
    return Object.entries(props.room.players).sort((a, b) => a[1].name - b[1].name)
  })

  const changeName = async e => {
    let room_name = e.target.value
    let response = await postData(`${urls.fetch}/api/room/rename/${props.room_id}`, {
      room_name,
        ...
        <div>
          <h1>
            <Show when={props.mode === 'advanced'}>
              <input class={styles.input} onChange={changeName} value={props.room.room_name} />
            </Show>
