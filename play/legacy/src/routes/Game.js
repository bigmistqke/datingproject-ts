...
function Game({ socket, user_id }) {
    ...
    for (let instruction of instructions) {
      if (instruction.type === 'video') {
        ...
        promises.push(new Promise((resolve, reject) => {
          ...
          console.log('instruction_text is: ', instruction.text);
          if (!instruction.text) {
            alert('video is missing');
            return;
          }
          xhrReq.open('GET', `${urls.fetch}${instruction.text}`, true);
  ...
  const getDesigns = async () => {
    let result = await fetch(`${urls.fetch}/api/design/get/oldie_2`);
  ...
  const joinRoom = async () => {
    ...
    let result = await fetch(`${urls.fetch}/api/room/join/${game_url}`);
    if (!result) {
  ...
  const init = async () => {
    ...
    window.onbeforeunload = async function () {
      let result = await fetch(`${urls.fetch}/api/room/disconnect/${room_url}/${role_url}`);
      socket.send(`${room_url}/${role_id}/status`, JSON.stringify({ role_url, status: 'disconnected' }));
    }
    setRender(performance.now());
  }
